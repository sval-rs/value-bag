...
- `error`: Enable support for capturing `std::error::Error`s. Implies `std` and `error-core`.
- `error-core`: Enable support for capturing `core::error::Error`s. Requires a version of Rust where `core::error` is stable unless the `std` feature is also enabled.
